...
# TODO: Determine whether nonviolent qda model is equally predictive across race, age, and gender
...
tab <- table(violent.qda.train$class, train$is_recid)
tab

# Training Accuracy for Violent QDA model is 2021/3703 = 54.577% (optimistic estimate)

# Test the Violent QDA model
violent.qda.test = predict(violent.qda, newdata=test)
tab <- table(violent.qda.test$class, test$is_recid)
tab

# Test Accuracy for Violent QDA model is 1342/2469 = 54.354% (note that this is less than the training accuracy)


# TODO: Determine whether violent qda model is equally predictive across race, age, and gender




# Run a Cox model to compare the accuracy of QDA models to COMPAS' model
...
# RAI
```{r}
rai <- function(type=c("lda", "qda", "rf", "dec", "reg")) {
  type <- match.arg(type)
  if (type == "lda") {
  }
}
