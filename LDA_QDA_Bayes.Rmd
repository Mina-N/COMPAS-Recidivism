...
# LDA Analysis
```{r}
# TODO: Feature selection/see what variables are important/build density plots
...
# Run a Cox model to compare the accuracy of the nonviolent LDA model to COMPAS' model
nonviolent.lda.test$class
...
# Write nonviolent lda predictions to cox-parsed-nonviolent-lda.csv 
...
write.csv(cox_parsed_lda, "cox-parsed-nonviolent-lda.csv")

data <- filter(filter(read.csv("./cox-parsed-nonviolent-lda.csv"), score_text != "N/A"), end > start) %>%
        mutate(race_factor = factor(race,
...
# TODO: Determine whether the model is equally predictive across race, age, and gender
...
# Training Accuracy for Violent LDA model is 1947/3703 = 52.58% (optimistic estimate)
...
# TODO: Run a Cox model to compare the accuracy of the violent LDA model to COMPAS' model
